    ...
    def localUpdate(self, localEpoch, localBatchSize, Net, lossFun, opti, global_parameters):
        ...
        for epoch in range(localEpoch):
            for data, label in self.train_dl:
                # 加载到GPU上
                data, label = data.to(self.dev), label.long().to(self.dev)
                # 模型上传入数据
                preds = Net(data)
                # 计算损失函数
                '''
...
class ClientsGroup(object):
    ...
    def __init__(self, dataSetName, isIID, numOfClients, dev):
        ...
        if self.data_set_name == "mnist":
            self.mnist_dataSetBalanceAllocation()
        else:
            self.cifar10_dataSetBalanceAllocation()

    def mnist_dataSetBalanceAllocation(self):
    ...
    def cifar10_dataSetBalanceAllocation(self):
        ...
        # np.random.permutation 将序列进行随机排序
        # np.random.permutation(50000//250=200)
